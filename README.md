...
<details>
  <summary>강의 관리 (강의 등록, 수정, 상태 관리)</summary>
...
<details>
  <summary>고종현</summary>
학습 기간이 길지 않고 비전공이라 스스로 확신이 없었지만, 일단 프로젝트를 시작했다. 팀과 함께 ERD를 만들어 보고, SQL을 직접 써 보고, GitHub도 사용해 보니 개념들이 조금씩 연결됐다. 특히 SQL과 GitHub는 부족했지만, 실전에서 써 보고 피드백을 주고받으면서 이해가 많이 나아졌다. 논의할 때는 각자 의견을 나누고, 서로의 결과물을 함께 보며 헷갈린 부분을 바로 정리했다. 잘못 알았던 내용은 그 자리에서 수정했고, 비판보다는 사실 확인과 수정을 우선했다. 팀원들은 맡은 일을 책임감 있게 진행했고, 먼저 끝난 사람은 다른 작업을 도와 전체 일정에 큰 문제 없이 마무리할 수 있었다.
</details>

<details>
  <summary>김현수</summary>
하나의 팀이 되어서 프로젝트를 진행한 것 같아 뿌듯합니다. 보통, 독립적인 일을 각 팀원에게 분담 받아서 일을 하는데, 우리 팀은 하나의 의사 결정을 할 때마다 모두가 참여해서 효율적인 팀플레이가 가능했던 것 같습니다.
</details>

<details>
  <summary>성현제</summary>
너무나 참담한 경우였습니다. 제가 노력을 안해서 그런건지는 몰라도 sql을 작성하면서 단 한번도 인공지능 없이 작성할 수 없었던 나를 보면서 참 지옥같았습니다. 만약 인공지능에게 감정이 있었다면 인공지능은 가히 벽을 설득하는 기분이었으리라 짐작해봅니다. 팀원들에게 참으로 미안한 상황이 많이 연출되었고, 처음에는 팀원들에게 의지하는 것이 너무나 힘든 일이었으나 다들 이런 시행착오가 있어서 지금의 위치에 있음을 말해줬고, 나도 언젠가는 이런 시절을 회상해볼 것이라고 첨언해주는 것들을 팀플을 진행하면서 듣다보니 왜인지모르게 점점 팀원들에게 의지하게 되었습니다. 
그런데 그것도 어느정도가 있었던 것 같습니다. 팀원들과 함께 있을때는 천군만마를 얻은듯 계속해서 일이 진행되었지만, 집으로 와서는 잘 진행이 안되고 벽에 막혀 그만두기도 했습니다. 그리고 집에 와서 했던일들을 쭉 생각해보니 팀원들과 함께 있을때 일이 진행되는 것은 제 실력이 늘어나는 것이 아니었던 것 같습니다. 만약 팀원들이 조금이라도 귀찮아 했더라면 앞으로 프로젝트에서도 만약 이런모습을 계속 보이게 된다면 사실 매니저님이 말했던 실제 프로젝트상에 이름만 올린 존재가 될지도 모른다는 생각이 들었습니다. 그래서 오히려 미리 매를 맞아서 다행인 것 같습니다. 앞으로도 매를 계속해서 맞겠지만 매가 맞은 이유를 계속해서 메우다보면 언젠가는 저도 매를 안맞고 어쩌다보면 매를 때리는 입장이 될지도 모릅니다. 그날이 올때까지 정진하겠습니다.
</details>

<details>
  <summary>박규진</summary>
  그동안 ERD나 SQL을 개별적으로 구현해 본 적은 있었지만, 이번 프로젝트는 아이디어 기획부터 요구사항 명세, ERD 설계, SQL 쿼리 구현까지 전 과정을 스스로 계획하고 수행한 첫 경험이었습니다. 
어려움도 있었지만 팀원 모두가 각자의 역할을 성실히 수행한 덕분에 정해진 기간 안에 의미 있는 결과물을 도출할 수 있었습니다.
</details>

<details>
  <summary>김명진</summary>
 팀 프로젝트를 열정 있는 분들과 해본 것이 처음이라 많이 배우고 성장한 것 같습니다. 주제 부터 모두 의견 나누고 프로젝트 마지막 단계까지 모두의 의견을 적절히 반영해서 팀 프로젝트는 이렇게 하는 구나를 깨달았습니다. 데이터베이스 구축을 이렇게 꼼꼼하게 진행했던 적이 없어서 SQL을 많이 공부하면서 할 수 있었습니다. 프로시저와 트리거는 배울 때는 와닿지 않았지만 프로젝트를 진행하면서 사용하고 테스트 하는 과정에서 정확히 알 수 있었습니다. 첫 프로젝트에 좋은 팀원들을 만나서 힘듬 없이 잘 진행해서 프로젝트의 부담감이 줄어들었습니다.
</details>
